...
3. Dependencies installed: FastAPI, Celery, Redis, WhisperX, Vue 3, Vite
4. Git repository configured with .gitignore for Python and Node
5. Basic README with setup instructions
6. Local development servers can run (backend on port 8000, frontend on port 5173)
...
3. Returns unique job_id for tracking
4. Saves uploaded file to server storage with job_id reference
5. Returns 400 error with clear message for unsupported formats
6. Handles files up to 2GB size
7. API endpoint documented in FastAPI auto-docs (/docs)
...
4. Task updates progress in Redis (0%, 25%, 50%, 75%, 100%)
5. Task stores transcription result as JSON with subtitle segments [{start, end, text}]
6. Task handles transcription errors and stores error state
...
5. Seeking works smoothly (Range request support validated)
6. Responsive layout: player scales appropriately on mobile/tablet
7. Player state persists during subtitle editing (doesn't reload unnecessarily)
...
7. Edits persist in browser memory during session (no auto-save to server yet)
...
**Story 2.7: End-to-End Integration Testing and Polish**

As a developer,
I want to validate the complete MVP workflow and fix any integration issues,
So that users have a reliable, polished experience.
